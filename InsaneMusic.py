...
class InsMusic(loader.Module):
    ...
    def get_track_key(self, msg):
        """Получить уникальный ключ трека для дедупликации"""
        if msg.audio:
            # Используем комбинацию title и performer как ключ
            title = msg.audio.title or ""
            performer = msg.audio.performer or ""
            duration = msg.audio.duration or 0
            return f"{performer}|{title}|{duration}"
        elif msg.document:
            # Для документов пробуем извлечь из атрибутов
            for attr in msg.document.attributes:
                if hasattr(attr, 'title') and attr.title:
                    ...
                    return f"{performer}|{title}|{duration}"
            # Если не получилось, используем имя файла
            if msg.file and msg.file.name:
                return f"file|{msg.file.name}"
    ...
    async def миcmd(self, message):
        ...
        try:
            await message.delete()
            searching_message = await message.respond(f"Ищу {search_query} в @lyadownbot...")
            ...
            self.active_searches[search_id] = {
                ...
                'results': [],
                'message_ids': [],
                'unique_tracks': set()  # Для отслеживания уникальных треков
            }
            ...
            async for msg in self.client.iter_messages(bot_entity, limit=30):
                if msg.id <= sent_msg.id:
                    continue
                
                # Проверяем, есть ли аудио в сообщении
                if msg.audio or msg.document:
                    # Получаем уникальный ключ трека
                    track_key = self.get_track_key(msg)
                    
                    # Проверяем, не сохранили ли мы уже это сообщение и уникален ли трек
                    if (msg.id not in self.active_searches[search_id]['message_ids'] and 
                        track_key not in self.active_searches[search_id]['unique_tracks']):
                        
                        self.active_searches[search_id]['results'].append(msg)
                        self.active_searches[search_id]['message_ids'].append(msg.id)
                        self.active_searches[search_id]['unique_tracks'].add(track_key)
                        
                        # Ограничиваем до 10 уникальных результатов
        ...
        if not message.out and message.sender:
            ...
            for search_id, search_data in list(self.active_searches.items()):
                ...
                if message.audio or message.document:
                    # Получаем уникальный ключ трека
                    track_key = self.get_track_key(message)
                    
                    # Проверяем уникальность и сохраняем результат
                    if (message.id not in search_data['message_ids'] and 
                        track_key not in search_data.get('unique_tracks', set())):
                        
                        search_data['results'].append(message)
                        search_data['message_ids'].append(message.id)
                        
                        if 'unique_tracks' not in search_data:
                            search_data['unique_tracks'] = set()
                        search_data['unique_tracks'].add(track_key)
