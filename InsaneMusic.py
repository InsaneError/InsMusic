...
class InsMusic(loader.Module):
    ...
    async def micmd(self, message):
        ...
        try:
            await message.delete()
            searching_message = await message.respond(f"<emoji document_id=5330324623613533041>⏰</emoji> Ищу в @lyadownbot...")
            ...
            for i, result in enumerate(results[:10]):  # Максимум 10 результатов
                ...
                # По 2 кнопки в ряд
                if len(row) == 2 or i == len(results[:10]) - 1:
                    buttons.append(row)
                    row = []
            ...
            # Сохраняем результаты в памяти (временно)
